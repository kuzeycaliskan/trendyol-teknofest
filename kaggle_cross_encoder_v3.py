"""
Cross-encoder v3 — KAGGLE NOTEBOOK'TA (GPU ile) çalıştırılır.

v2'den fark: eğitime pseudo_labels_ce2.csv eklendi — CE2 harmanının test
üzerindeki çok emin tahminleri (p>=0.99 pozitif / p<=0.01 negatif, terim
başına 25/10 üst sınırlı). Self-training riski (kendi hatasını betonlama)
sıkı eşik + üst sınır + orijinal veriyle karışım + TEK TUR ile yönetiliyor.
Validasyon kurulumu v2 ile BİREBİR AYNI bırakıldı (pseudo veri val'e girmez)
— skorlar v2 ile karşılaştırılabilir kalsın diye.
...
pseudo = pd.read_csv(find_input("pseudo_labels_ce2.csv"))
...
# Sözde etiketler yalnızca TRAIN'e (test terimleri val terimleriyle ayrık)
df_tr = pd.concat([df_tr, pseudo], ignore_index=True).sample(frac=1, random_state=SEED)
...
np.save(f"{OUT}/ce3_val_scores.npy", predict_chunked(df_va))
np.save(f"{OUT}/ce3_val_labels.npy", df_va.label.to_numpy())
df_va[["term_id", "item_id", "label"]].to_csv(f"{OUT}/ce3_val_pairs.csv",
                                              index=False)

print("[5/5] Test skorlanıyor (3.36M çift)...")
np.save(f"{OUT}/ce3_test_scores.npy", predict_chunked(test))
print("Bitti. İndirilecekler: ce3_test_scores.npy, ce3_val_scores.npy, "
      "ce3_val_labels.npy, ce3_val_pairs.csv")
