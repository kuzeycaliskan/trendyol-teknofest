"""
Cross-encoder v5 — Kaggle GPU'da VEYA Windows masaüstünde (RTX 4070) çalışır.

v2'den farklar (plato kırma denemesi — model sınıfı değişiyor):
- Taban: ytu-ce-cosmos/turkish-base-bert-uncased (daha büyük ve güncel Türkçe
  korpusla eğitilmiş; dbmdz'den farklı aile → ensemble çeşitliliği de sağlar).
  Not: mDeBERTa-v3 da düşünüldü ama fp16 eğitimde bilinen kararsızlığı var,
  sıkışık takvimde risk alınmadı.
- Ürün metni zenginleştirildi: attributes 200→400 karakter, max_length 128→160.
- epochs=2 (ce4'te kanıtlanan ölçek).
Validasyon kurulumu yine birebir aynı (ce2/3/4 ile karşılaştırılabilir).
Süre: 4070'te ~3 saat, T4'te ~7 saat.
...
np.save(f"{OUT}/ce5_val_scores.npy", predict_chunked(df_va))
np.save(f"{OUT}/ce5_val_labels.npy", df_va.label.to_numpy())
df_va[["term_id", "item_id", "label"]].to_csv(f"{OUT}/ce5_val_pairs.csv",
                                              index=False)

print("[5/5] Test skorlanıyor (3.36M çift)...")
np.save(f"{OUT}/ce5_test_scores.npy", predict_chunked(test))
print("Bitti. İndirilecekler: ce5_test_scores.npy, ce5_val_scores.npy, "
      "ce5_val_labels.npy, ce5_val_pairs.csv")
