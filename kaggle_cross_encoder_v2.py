"""
Cross-encoder v2 — KAGGLE NOTEBOOK'TA (GPU ile) çalıştırılır.
...
np.save(f"{OUT}/ce2_val_scores.npy", predict_chunked(df_va))
np.save(f"{OUT}/ce2_val_labels.npy", df_va.label.to_numpy())
df_va[["term_id", "item_id", "label"]].to_csv(f"{OUT}/ce2_val_pairs.csv",
                                              index=False)

print("[5/5] Test skorlanıyor (3.36M çift)...")
np.save(f"{OUT}/ce2_test_scores.npy", predict_chunked(test))
print("Bitti. İndirilecekler: ce2_test_scores.npy, ce2_val_scores.npy, "
      "ce2_val_labels.npy, ce2_val_pairs.csv")
