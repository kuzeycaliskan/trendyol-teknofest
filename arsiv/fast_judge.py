"""
HIZLI hakem — 1.5B + EN DAR bant (66k). CUDA'da ~15 dk, MPS'te ~1-2 saat.
GPU teyidi script başında basılır; "cuda: False" görürsen GPU kullanılmıyor.
...
print(f"CİHAZ: {DEV}  (cuda: {torch.cuda.is_available()})")
if DEV == "cpu":
    raise SystemExit("GPU yok -> CPU'da bitmez. DUR. Mac oturumuna bildir.")

MODEL = "Qwen/Qwen2.5-1.5B-Instruct"
CSV = "uncertain_tight.csv"
...
if os.path.exists("llm_tight_scores.npy"):
    scores = np.load("llm_tight_scores.npy")
    done = int(np.load("llm_tight_done.npy"))
...
with torch.inference_mode():
    for s in range(done, len(prompts), BATCH):
        ...
        if (s - done) % SAVE_EVERY < BATCH:
            np.save("llm_tight_scores.npy", scores)
            np.save("llm_tight_done.npy", np.array(s + len(batch)))
            np.save("llm_tight_rows.npy", df.row_idx.to_numpy())
            print(f"{s + len(batch)}/{len(prompts)} — kaydedildi")

np.save("llm_tight_scores.npy", scores)
np.save("llm_tight_done.npy", np.array(len(prompts)))
np.save("llm_tight_rows.npy", df.row_idx.to_numpy())
print("Bitti. İndir: llm_tight_scores.npy, llm_tight_rows.npy")
