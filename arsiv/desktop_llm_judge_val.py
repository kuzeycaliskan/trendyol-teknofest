"""
HAKEM DOĞRULAMA — bilinen etiketli 6000 zor çift (~2 dk). Duman testi + go/no-go.
...
if os.path.exists("llm_val_scores.npy"):
    scores = np.load("llm_val_scores.npy")
    done = int(np.load("llm_val_done.npy"))
...
with torch.inference_mode():
    for s in range(done, len(prompts), BATCH):
        ...
        if (s - done) % SAVE_EVERY < BATCH:
            np.save("llm_val_scores.npy", scores)
            np.save("llm_val_done.npy", np.array(s + len(batch)))
            np.save("llm_val_labels.npy", labels)
            print(f"{s + len(batch)}/{len(prompts)} — kaydedildi")

np.save("llm_val_scores.npy", scores)
np.save("llm_val_done.npy", np.array(len(prompts)))
np.save("llm_val_labels.npy", labels)
# yerel go/no-go: bilinen etiketlerde en iyi eşikte isabet
best = 0.0
for thr in np.arange(0.3,0.71,0.05):
    acc = ((scores>=thr).astype(int)==labels).mean()
    best = max(best, acc)
print(f"HAKEM İSABETİ (zor çiftler, en iyi eşik): {best:.3f}")
print(">=0.70 ise banda katmaya DEĞER; <0.62 ise GÖNDERME, 0.893 mühürle.")
print("Bitti. İndir: llm_val_scores.npy, llm_val_labels.npy")
