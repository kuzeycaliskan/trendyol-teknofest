"""
Cross-encoder v13 — masaüstü (RTX 4070), SON GECE MAX-CONFIG:
dbmdz + tüm veri + TÜM negatif sınıfları (random+mined+sameleaf) +
3 EPOCH + max_len 256 + attributes 800 karakter. Şimdiye kadar hız için
kısıtlanan iki ekseni (derinlik+genişlik) aynı anda açan tek koşu.
batch 32 (len 256 VRAM dengesi). Süre: ~5-6 saat — gece işi.
...
MAX_LEN = 256
ATTR_CHARS = 800  # attributes'ın ilk N karakteri (tokenizer zaten 128 tokene kırpar)
...
sameleaf = pd.read_csv(find_input("sameleaf_negatives.csv"))
...
def build_split(pos_df):
    ...
    sl = sameleaf[sameleaf.term_id.isin(set(pos_df.term_id))]
    df = pd.concat(
        [pos_df[["term_id", "item_id", "label"]], random_negatives(pos_df), m, sl],
        ignore_index=True,
    )
...
loader = DataLoader(train_examples, shuffle=True, batch_size=32, drop_last=True)
model.fit(train_dataloader=loader, epochs=3, warmup_steps=1500,
          use_amp=True, show_progress_bar=True)
model.save(f"{OUT}/ce13_model")
...
np.save(f"{OUT}/ce13_test_scores.npy", predict_chunked(test))
print("Bitti. İndirilecek TEK dosya: ce13_test_scores.npy")
